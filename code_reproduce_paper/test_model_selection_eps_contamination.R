## Figure 3: Model selection comparison (AIC, BIC, ICL, DSC) under
## epsilon-contamination misspecification (Section 5.2, Misspecified Regime 1).
## True: p0 = (1-eps)*[0.4*N(5,1) + 0.6*N(10,1.5^2)] + eps*Laplace(0,1), eps=0.01
...
gen_data <- function(n) {
  is_clean <- rbinom(n, 1, 1 - eps) == 1
  x <- numeric(n)
  nc <- sum(is_clean)
  if (nc > 0) {
    comp <- sample(2L, nc, replace = TRUE, prob = ps_clean)
    x[is_clean] <- ifelse(comp == 1L,
                          rnorm(nc, mus_clean[1], sds_clean[1]),
                          rnorm(nc, mus_clean[2], sds_clean[2]))
  }
  nco <- n - nc
  if (nco > 0) x[!is_clean] <- rlaplace(nco, 0, 1)
  x
}

true_density <- function(x) {
  g <- ps_clean[1] * dnorm(x, mus_clean[1], sds_clean[1]) +
       ps_clean[2] * dnorm(x, mus_clean[2], sds_clean[2])
  lap <- 0.5 * exp(-abs(x))   # Laplace(0, 1) density
  (1 - eps) * g + eps * lap
}
...
compute_dsc <- function(x, dmm, k_over) {
  ...
  heights  <- dmm$hc$height          # length k_over - 1; height[j] = j-th merge cost
  max_h    <- max(heights)
...
# AIC, BIC, ICL from fitting k = 1..k_max models separately
...
xgrid    <- seq(min(x_demo) - 1, max(x_demo) + 1, length.out = 500)
...
fig3 <- p_hist | p_dendro | p_avg

quartz(width = 15, height = 5)
print(fig3)

ggsave("code_reproduce_paper/R_fig3_eps_contamination.pdf",
       plot = fig3, width = 15, height = 5)
cat("\nSaved: code_reproduce_paper/R_fig3_eps_contamination.pdf\n")
